...
RSpec.describe Separatum::Processors::UuidChanger do
  ...
  describe '#transform_hash' do
    it { expect(subject.transform_hash(hash1)[:a]).to eq hash1[:a] }
    it { expect(subject.transform_hash(hash1)[:b]).to eq hash1[:b] }
    it { expect(subject.transform_hash(hash1)[:c]).to_not eq hash1[:c] }
  end

  describe '#transaform_hashes' do
